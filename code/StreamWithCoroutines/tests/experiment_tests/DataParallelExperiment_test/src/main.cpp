...
public:
  ...
  void runExperiment(int width, int numThreads, vector<double> costList, double selectivity, SchedulerPlugin * plugin, double & throughput, double & latency) {
    std::chrono::seconds timespan(5);
    std::this_thread::sleep_for(timespan);

    DataParallel DataParallel(costList, selectivity, width);
  ...
  void repeatExperiment(int numberOfRuns, int width, int numThreads, vector<double> costList, double selectivity, int schedulerId, int quanta, double & avgThroughput, double & throughputDev, double & avgLatency, double & latencyDev) {
    ...
    for(int i=0; i<numberOfRuns; i++) {
      ...
      runExperiment(width, numThreads, costList, selectivity, getScheduler(schedulerId, quanta), throughput, latency);
  ...
  void process() 
  {
    ...
    vector<double> costList = generateCosts(defaultCost, defaultWidth);
    ...
    for (size_t numThreads=numThreadsMin; numThreads<=numThreadsMax; ++numThreads) {
      ...
      for(size_t j=0; j<schedulers.size(); j++) {
        repeatExperiment(numberOfRuns, defaultWidth, numThreads, costList, defaultSelectivity, j, defaultQuanta, throughput, throughputDev, latency, latencyDev);
...
/*
    ...
    for (size_t cost=minCost; cost<=maxCost; cost+=20) {
      ...
      for(size_t j=0; j<schedulers.size(); j++) {
        repeatExperiment(numberOfRuns, defaultWidth, defaultThreads, cost, defaultSelectivity, j, defaultQuanta, throughput, throughputDev, latency, latencyDev);
